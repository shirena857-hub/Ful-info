...
region_cache = TTLCache(maxsize=200, ttl=3600)  # 1 hour per UID
...
# === Auto-detect region for a UID ===
async def fetch_region_from_uid(uid: str) -> str:
    """Try all supported regions until we find one that returns valid account data."""
    uid = str(uid)
...
# === Formatting function (only for /player-info) ===
...
# /player-info – formatted response
...
# /get – raw JSON (exactly as your snippet) with auto‑detection and caching
@app.route('/get')
def get_account_info_raw():
    ...
    # If we already know the region, use it
    if uid in region_cache:
        try:
            return_data = asyncio.run(GetAccountInformation(uid, "7", region_cache[uid], "/GetPlayerPersonalShow"))
            formatted_json = json.dumps(return_data, indent=2, ensure_ascii=False)
            return formatted_json, 200, {'Content-Type': 'application/json; charset=utf-8'}
        except Exception:
            pass  # fall through to try all regions
