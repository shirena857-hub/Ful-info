...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=5000, debug=True)    padding_length = AES.block_size - (len(text) % AES.block_size)
    return text + bytes([padding_length] * padding_length)
...
# নতুন এন্ডপয়েন্ট: ফরম্যাটেড ডেটা
@app.route('/get')
@cached_endpoint()
def get_account_info_formatted():
    uid = request.args.get('uid')
...
# পুরনো এন্ডপয়েন্ট: Raw JSON (আগের মতো)
@app.route('/player-info')
@cached_endpoint()
def get_account_info_raw():
    uid = request.args.get('uid')
